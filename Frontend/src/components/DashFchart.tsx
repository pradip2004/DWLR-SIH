...
const options = {
  ...
  plugins: {
    ...
    tooltip: {
      callbacks: {
        label: (tooltipItem) => `Value: ${tooltipItem.raw}`, // Customize tooltip text
      },
    },
  },
  elements: {
...
function DashFchart({ selectedState, selectedCity }) {
  ...
  // Avoid rendering charts if data is loading
  if (loading) {
  ...
  return (
    <div className="w-full h-1/2 shadow-xl mb-2 rounded-md flex flex-col overflow-hidden">
      {/* State and City Name */}
      <div className="bg-[#274C77] px-4 py-2">
        <h3 className="text-md font-semibold text-white">{selectedState || 'State Name'}</h3>
        <h3 className="text-md text-zinc-200">{selectedCity || 'City Name'}</h3>
      </div>
      ...
      <div className="flex justify-center items-center w-full">
        {chartType1 === 'pie' ? (
          <Pie
            data={pieData}
            options={options}
            className="w-full h-48"  // Decreased chart size and added max-width for responsiveness
          />
        ) : (
          <Bar
            data={barData}
