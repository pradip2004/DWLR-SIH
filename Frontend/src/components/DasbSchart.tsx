...
function DashSchart({selectedState, selectedCity}) {
  ...
  return (
    <div className="w-full h-1/2 shadow-xl rounded-md overflow-hidden flex flex-col">
      {/* State and City Name */}
      <div className="px-4 py-2 bg-[#274C77]">
        <h3 className="text-lg font-semibold text-white">{selectedState || 'State Name'}</h3>
        <h3 className="text-sm text-zinc-200">{selectedCity || 'City Name'}</h3>
      </div>
      ...
      <div className="flex justify-between gap-4 px-4 py-2">
        <button
          ...
          className="text-[#274C77] border border-[#274C77] py-1 px-4 rounded-md text-sm font-medium flex items-center gap-2 group hover:bg-[#274C77] hover:text-white"
        >
      ...
      <div className="flex justify-center items-center w-full">
        {chartType1 === 'pie' ? (
          <Pie
            data={pieData}
            options={options}
            className="w-1/2 h-48"  // Decreased chart size
          />
        ) : (
          <Bar
            data={barData}
