...
import FutureBtteryLevel from "../components/FutureBtteryLevel";
...
const Analytics = () => {
  ...
  const handleSearchChange = (e) => {
    ...
    if (validDWLRIds.includes(Number(query))) {
      setSelectedNumber(Number(query)); // Set selected number if valid DWLR ID
    } else {
  ...
  return (
    <div className="main w-full min-h-screen mb-12 overflow-scroll bg-[#DEFFFC] p-5">
      ...
      <div className="flex space-x-4 mt-8 gap-5 items-center">
        ...
        <div className="relative flex items-center">
          <input
            ...
            placeholder="Search Number (1 or 2)..."
          />
        ...
        <div className="relative w-[250px]">
          ...
          >
            ...
            <span>{selectedState ? indianStates.find(state => state.id === selectedState)?.name : 'State'}</span>
            <MdArrowDropDown
      ...
      <div className="water-level w-full h-[82vh] flex gap-10 justify-between mt-10">
        ...
        <CurrentWater waterLevel={selectedNumber === 1 ? 60 : selectedNumber === 2 ? 50 : 0} />
        <FutureWater waterLevel={selectedNumber === 1 ? 65 : selectedNumber === 2 ? 55 : 0} />
      </div>
